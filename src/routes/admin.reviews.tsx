  ...
  return (
    <>
      ...
      <div className="grid gap-4 md:grid-cols-2">
          ...
          <Panel key={r.id} className="p-5">
            <div className="grid gap-3 sm:grid-cols-2">
              <Field label="Автор">
                <TextInput
                  ...
                  onChange={(e) =>
                    setReviews((p) =>
                      p.map((x) => (x.id === r.id ? { ...x, author: e.target.value } : x)),
                    )
                  }
                />
              </Field>
              <Field label="Уровень">
                <TextInput
                  ...
                  onChange={(e) =>
                    setReviews((p) =>
                      p.map((x) => (x.id === r.id ? { ...x, level: e.target.value } : x)),
                    )
                  }
                />
              </Field>
            ...
            <div className="mt-3">
              <PhotoPicker
                ...
                value={r.photo ?? ""}
                onChange={(photo) =>
                  setReviews((p) => p.map((x) => (x.id === r.id ? { ...x, photo } : x)))
                }
              />
            </div>
            <div className="mt-3">
            ...
            <div className="mt-3 flex items-center justify-between">
              <Toggle
                ...
                onChange={(v) =>
                  setReviews((p) => p.map((x) => (x.id === r.id ? { ...x, visible: v } : x)))
                }
              />
              <Btn
                variant="ghost"
        ...
        {reviews.length === 0 ? (
          <div className="py-10 text-sm text-[oklch(0.55_0.03_45)]">Отзывов пока нет</div>
        ) : null}
      </div>
