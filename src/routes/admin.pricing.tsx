    ...
    <>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {prices.map((p) => (
          <Panel
            key={p.id}
            className={`p-5 ${p.featured ? "ring-2 ring-[oklch(0.8_0.15_75)]" : ""}`}
          >
            <div className="mb-3 flex items-center justify-between gap-2">
              <span className="text-xs font-semibold uppercase tracking-wide text-[oklch(0.6_0.03_45)]">
                Порядок на сайте
              </span>
            ...
            <div className="space-y-3">
              <Field label="Название">
                <TextInput
                  ...
                  onChange={(e) =>
                    setPrices((v) =>
                      v.map((x) => (x.id === p.id ? { ...x, title: e.target.value } : x)),
                    )
                  }
                />
              </Field>
              <div className="grid grid-cols-2 gap-3">
                <Field label="Цена">
                  <TextInput
                    ...
                    onChange={(e) =>
                      setPrices((v) =>
                        v.map((x) => (x.id === p.id ? { ...x, price: e.target.value } : x)),
                      )
                    }
                  />
                </Field>
                <Field label="Период">
                  <TextInput
                    ...
                    onChange={(e) =>
                      setPrices((v) =>
                        v.map((x) => (x.id === p.id ? { ...x, period: e.target.value } : x)),
                      )
                    }
                  />
                </Field>
              ...
              <details className="rounded-2xl border border-[oklch(0.92_0.02_60)] p-3">
                <summary className="cursor-pointer text-sm font-semibold">
                  Детали для страницы «Все программы»
                </summary>
                <div className="mt-3 space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <Field label="Иероглиф">
                      <TextInput
                        value={p.hanzi ?? ""}
                        onChange={(e) => patch(p.id, { hanzi: e.target.value })}
                      />
                    </Field>
                    <Field label="Подзаголовок">
                      <TextInput
                        value={p.tag ?? ""}
                        onChange={(e) => patch(p.id, { tag: e.target.value })}
                      />
                    </Field>
                    <Field label="Формат">
                      <TextInput
                        value={p.format ?? ""}
                        onChange={(e) => patch(p.id, { format: e.target.value })}
                      />
                    </Field>
                    <Field label="Размер группы">
                      <TextInput
                        value={p.groupSize ?? ""}
                        onChange={(e) => patch(p.id, { groupSize: e.target.value })}
                      />
                    </Field>
                    ...
                    <Field label="Уровень">
                      <TextInput
                        value={p.level ?? ""}
                        onChange={(e) => patch(p.id, { level: e.target.value })}
                      />
                    </Field>
                  </div>
                  <Field label="Итоговая фраза">
                    <TextArea
                      className="min-h-16"
                      value={p.footer ?? ""}
                      onChange={(e) => patch(p.id, { footer: e.target.value })}
                    />
                  </Field>
                  <Field label="Плашка (акция)">
                    <TextInput
                      value={p.highlight ?? ""}
                      onChange={(e) => patch(p.id, { highlight: e.target.value })}
                    />
                  </Field>
                </div>
              </details>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                <Toggle
                  ...
                  label="На сайте"
                  onChange={(val) =>
                    setPrices((v) => v.map((x) => (x.id === p.id ? { ...x, visible: val } : x)))
                  }
                />
                <Btn
                  size="sm"
                  ...
                  onClick={() =>
                    setPrices((v) =>
                      v.map((x) => ({ ...x, featured: x.id === p.id ? !x.featured : x.featured })),
                    )
                  }
                >
                ...
                <Btn
                  ...
                  onClick={() => setPrices((v) => v.filter((x) => x.id !== p.id))}
                >
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Формат">
              <TextInput
                value={draft.format}
                placeholder="Онлайн, мини-группа"
            ...
            <Field label="Размер группы">
              <TextInput
                ...
                placeholder="До 4 человек"
                onChange={(e) => setDraft({ ...draft, groupSize: e.target.value })}
              />
            </Field>
            <Field label="Длительность">
              <TextInput
